...
`
function Office() {
    const [data,setData] = useState({officeName:"",Code:"",Description:""})
    const getContext = useContext(Context);
    const {t,load,find} = getContext;
    const TableHeading = [t("Order"),t("OfficeName"),t("Code"),t("Describtion"),t("Action")];
    const [Offices] = useGetData(`http://localhost:5000/getOfficeList?find=${find}`)
    const [res,HandleSubmit] = usePostData({url:'http://localhost:5000/AddOffice', Data:data});
  return <>
   <Navbar/>
      ...
      <div className='sm-countainer'>
        ...
        <div className="input-control">
              <label htmlFor="office-name">{t("OfficeName")}</label>
               <input type="text" id='office-name' onChange={(e)=>setData({...data,officeName:e.target.value})}/>
              </div>
            </div>
            <div className='Form-Control'>
            <div className="input-control">
              <label htmlFor="Code">{t("Code")}</label>
               <input type="text" id='Code' onChange={(e)=>setData({...data,Code:e.target.value})}/>
              </div>
            </div>
            <div className="Form-Control">
          <div className="input-control">
