...
const Td = styled.td`
  padding: 1.5rem;
`;
const UsedPropertyCost = () => {
  const getContext = useContext(Context);
  const { t, load, find } = getContext;
  const [Vechiles] = useGetData(
    `http://localhost:5000/getUsedPropertyCost?find=${find}`
  );
  const TableHeading = [
    t("Order"),
    t("FileNumber"),
      ...
      <div className="countainer">
        ...
        <main className="content">
          <div className="content-countainer">
            <PageHeader Message={[t("AccountsPayableReceipt2")]} />
            <RegistorButton
              Message={[t("CreateNew22ሐModel"), "/NewUsedPropertyCost"]}
            />
          </div>
...
export default UsedPropertyCost;
