...
function DepartmentType() {
    ...
    const [res,HandleSubmit] = usePostData({url:'http://localhost:5000/AddDepartmentType', Data:data});
    useEffect(()=>{
        console.log(data)
    },[data])
  return <>
   ...
   <PageHeader Message= {[t("NewDepartmentType")]} />
        ...
        <form action="">
            ...
            <div className="input-control">
              <label htmlFor="Department">{t("Department")}</label>
               <input type="text" id='Department' onChange={(e)=>setData({...data,departmentType:e.target.value})}/>
              </div>
            </div>
            <Submit>
    ...
    <Find/></div>
            <div className='Table' style={{marginRight:"1rem"}}>
              ...
              <tbody>
                   {DepartmentsType?.slice(0,load?load:5).map((data, index) => (
                        <tr key={index}>
                            <td>{index+1}</td>
